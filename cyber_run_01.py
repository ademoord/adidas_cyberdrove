...
def play_start_sound():
    start_sound = Sound('static/sound/testtt_dongse.mp3')
    start_sound.play()
...
while True:
        ...
        if event.type == MOUSEBUTTONDOWN:
            if not running:
                if event.button == 3:  # Right mouse button
                    ...
                    countdown_start_time = None  # Reset countdown_start_time for new attempt
                    play_start_sound()
                continue
    ...
    if running and time.time() - game_start_time > game_duration:
        ...
        click_intervals = []
        current_main_video = main_videos['start']
        current_side_video = side_videos['start']  # Fix: Reset to side_saver instead of face_saver
    ...
    if running:
        ...
        y_offset_speed = screen_height // 2 - len(lines_speed) * 115  # Adjusted for larger text
